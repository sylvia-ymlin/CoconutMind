...
from model.model import CoconutMindConfig  # 模型配置类
...
def train_epoch(epoch, loader, total_steps, start_step=0, wandb=None):
    ...
    :param start_step: 开始训练的步数 -> 用于断点续训
    :param wandb: wandb对象
    '''

    loss_fn = nn.CrossEntropyLoss(reduction='none')  # 定义交叉熵损失函数
    start_time = time.time()  # 记录开始时间
    # 前向传播
    # step 从 start_step + 1 开始计数
    for step, (x, y, loss_mask) in enumerate(loader, start_step = start_step + 1):
        # 将数据移动到指定设备
        x, y, loss_mask = x.to(args.device), y.to(args.device), loss_mask.to(args.device)

        lr = get_lr(epoch * total_steps + step, args.epochs * total_steps, args.learning_rate) # 计算当前学习率

        for group in optimizer.param_groups:
            group['lr'] = lr  # 更新优化器的学习率
        ...
        with autocast_ctx:  # 混合精度上下文
            ...
            # 计算损失
            # shape 的对齐
            # logits 的 shape: (batch_size, seq_len, vocab_size)
            # y 的 shape: (batch_size, seq_len)
            # 对齐：logits 最后一维不变，前两维展平
            # y 直接展平
            # 计算交叉熵损失
